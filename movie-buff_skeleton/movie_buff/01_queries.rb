...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(castings: :actor).where(actors: { name: 'Harrison Ford' }).where.not(castings: { ord: 1 } )
  
  #Movie.select(:id, :title).joins(castings: :actor).where('actors.name = \'Harrison Ford\' AND castings.ord != 1')
  #This also works ^^^
end
...
def directed_by_one_of(them)
  ...
  # map { |director| castings.actor_id.find_by(actors.name = director) }
end
